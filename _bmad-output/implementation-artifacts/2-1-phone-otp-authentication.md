...
- [x] [Review][Defer] `account.tsx`'s `signOut()` only clears local storage (AsyncStorage + SecureStore) with no server-side session revocation call — deferred, pre-existing. Currently zero real consequence: no screen anywhere calls `signOut()` (confirmed via repo-wide grep), so this is unreachable dead code, not an active gap. Worth fixing when a sign-out UI is actually built, not before.
