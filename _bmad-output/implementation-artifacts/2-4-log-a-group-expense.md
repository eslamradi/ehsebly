...
- [x] **Enhancement (2026-07-30, post-`done`):** Admin expense editing/deletion — real production request ("expenses details should be shown and editable by group admin"). "Admin" has no dedicated role (decision: the group's creator, `groups.created_by_user_id`, is its one permanent admin — matches how `createGroup` already made the creator the first active member; zero schema change needed for the role itself).
  - Migration `0003_add_expense_rate_columns.sql`: adds `tax_enabled`/`tax_rate_percent`/`service_enabled`/`service_rate_percent`/`other_service_enabled`/`other_service_rate_percent` to `expenses`. These were already received on submit (previous bullet) but discarded after arithmetic verification — editing needs the *original* inputs back, since 0 `service_piastres` is ambiguous between "disabled" and "0%, enabled" (lossy to reconstruct from amounts alone). Applied to staging D1; local/prod still pending whenever those environments are next touched.
  - Backend: `authMiddleware.ts` gains `requireGroupAdmin` (checks `groups.created_by_user_id`). `db/expenses.ts` gains `getExpenseGroupId`/`updateExpense`/`deleteExpense` (full item/assignment replace-not-diff on edit, matching `insertExpense`'s all-at-once shape; no `ON DELETE CASCADE` on the FKs so children are deleted in explicit order). `routes/groups.ts`'s validation logic (member-id + arithmetic checks) extracted into a shared `validateExpenseAgainstGroup`, reused by both `submitExpenseRoute` and the new `updateExpenseRoute` — an edit can't skip a check a fresh submission can't. New routes: `POST /groups/:groupId/expenses/:expenseId` (update), `POST /groups/:groupId/expenses/:expenseId/delete` (delete), both `requireAuth` → `requireGroupMember` → `requireGroupAdmin`.
  - Client: new `ExpenseDetailScreen` (reached by tapping a row in Story 2.5's expense list) shows the full item/assignment/tax-service breakdown; Edit/Delete only rendered for the admin (server independently re-enforces regardless). New `ExpenseEditScreen` — a single-form editor (add/remove items, per-item member assignment chips, payer picker, tax/service toggles+rates, live total preview) reusing `calculateSplitTotals`/`parseEGPToPiastres`/`parsePercentInput` rather than re-deriving validation. Delete uses a two-tap "tap again to confirm" pattern (no `Alert.alert` anywhere else in this codebase to match). `domain/group.ts`'s `Group` gains `createdByUserId`; `domain/groupLedger.ts`'s `GroupExpense` gains optional `id`/`name`/`quantity`/`isShared` on items and optional tax/service rate fields (additive — `verifyGroupBalances.ts`'s fixtures unaffected).
  - Deployed to staging (migration applied + `wrangler deploy --env staging`). Verified with `tsc --noEmit` from both `client/` and `client/backend/worker/` (clean).
